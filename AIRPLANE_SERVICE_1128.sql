...
INSERT INTO PLANE VALUES ('1','00001',5,5,30);
...
INSERT INTO COUNTRY VALUES ('0001', '0001', 200, 4.5);
INSERT INTO COUNTRY VALUES ('0002', '0002', 200, 3);
...
PRICE NUMBER(8),                         --가격
DEPARTURE_HOUR NUMBER(4 ,2) NOT NULL,            --출발 시간
ARRIVAL_HOUR NUMBER(4, 2) NOT NULL               --도착 시간
);
...
UPDATE FLIGHT SET PLANE_NO = TO_CHAR(2,'0000'), ARRIVAL_COUNTRY_NO = 0001, DEPARTURE_HOUR = 5 WHERE NO = TO_CHAR(3,'0000');

INSERT INTO FLIGHT(NO,PLANE_NO,ARRIVAL_COUNTRY_NO,DEPARTURE_HOUR) VALUES(TO_CHAR(1,'0000'),TO_CHAR(1,'0000'),'0001',2);
INSERT INTO FLIGHT(NO,PLANE_NO,ARRIVAL_COUNTRY_NO,DEPARTURE_HOUR) VALUES(TO_CHAR(2,'0000'),TO_CHAR(1,'0000'),'0002',2);
DELETE FROM FLIGHT WHERE NO = LPAD(1, 4,'0');
DELETE FROM FLIGHT WHERE NO = TO_CHAR(2,'0000');
...
ALTER TABLE FLIGHT ADD CONSTRAINT FLIGHT_DEPARTURE_COUNTRY_NO_FK FOREIGN KEY(DEPARTURE_COUNTRY_NO) REFERENCES COUNTRY(NAME) ON DELETE SET NULL;
ALTER TABLE FLIGHT ADD CONSTRAINT FLIGHT_ARRIVAL_COUNTRY_NO_FK FOREIGN KEY(ARRIVAL_COUNTRY_NO) REFERENCES COUNTRY(NAME) ON DELETE SET NULL;

ALTER TABLE FLIGHT drop CONSTRAINT FLIGHT_PLANE_NO_FK;
ALTER TABLE FLIGHT drop CONSTRAINT FLIGHT_DEPARTURE_COUNTRY_NO_FK;
ALTER TABLE FLIGHT drop CONSTRAINT FLIGHT_ARRIVAL_COUNTRY_NO_FK;

ALTER TABLE FLIGHT MODIFY DEPARTURE_COUNTRY_NO CHAR(5) DEFAULT TO_CHAR(1,'00000');
ALTER TABLE FLIGHT MODIFY ARRIVAL_COUNTRY_NO CHAR(5);
--항공편 시퀀스
...
--시간대비 가격 자동 계산 트리거
CREATE TRIGGER FLIGHT_PRICE_TIRRGER
...
CREATE TRIGGER FLIGHT_PRICE_TIRRGER2
...
--시간대비 도착시간 자동 계산 트리거
CREATE TRIGGER FLIGHT_ARRIVAL_TIRRGER
BEFORE INSERT ON FLIGHT
FOR EACH ROW
DECLARE
    SHOUR NUMBER(4.2);
BEGIN
    SELECT HOUR INTO SHOUR FROM COUNTRY C WHERE C.NO = :NEW.ARRIVAL_COUNTRY_NO;
    :NEW.ARRIVAL_HOUR := :NEW.DEPARTURE_HOUR + SHOUR;
END;
/
...
CREATE TRIGGER FLIGHT_ARRIVAL_TIRRGER2
BEFORE UPDATE ON FLIGHT
FOR EACH ROW
DECLARE
    SHOUR NUMBER(4.2);
...
select * from flight f inner join country c on ARRIVAL_COUNTRY_NO = name;
SELECT HOUR FROM COUNTRY C INNER JOIN FLIGHT F ON C.NO = F.ARRIVAL_COUNTRY_NO;
 commit;